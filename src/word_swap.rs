...
pub struct WordSwap{
    to_be_swapped_word:&'static str,
    swap_with:&'static str,
...
impl WordSwap {
    ...
    pub fn new(word: &'static str,to_be_swapped_word:&'static str,swap_with:&'static str) -> Self {
        WordSwap{
            to_be_swapped_word:to_be_swapped_word,
            swap_with:swap_with,
    ...
    pub fn swap(&mut self) -> String {
        self.set_split_word();
        if self.split_word.contains(&self.to_be_swapped_word.to_string()) {
            for index in 0..self.split_word.len() {
                if self.split_word[index].eq(&self.to_be_swapped_word.to_string()) {
                    self.split_word[index] = self.swap_with.to_string();
                }
            }
        }else {
            return String::from("");
